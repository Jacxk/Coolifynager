...
// Memoized component for individual resource cards
const MemoizedResourceCard = memo(ResourceCard);

// Memoized component for skeleton cards
...
export const FavoritesList = memo(function FavoritesList({
  ...
  // Memoize the grouped data to prevent unnecessary re-renders
  const grouped = useMemo(() => groupBy(data, (obj) => obj.type), [data]);
  ...
  const presentTypes = useMemo(() => 
    typeOrder.filter(({ type }) => grouped[type]), [typeOrder, grouped]
  );
  ...
  const renderResourceCard = useCallback((item: ResourceData) => (
            ...
            <View key={type} className="flex gap-2">
              ...
              <View className="p-2 gap-2">
                {groups[type as ResourceType].map((item) => (
                  <MemoizedSkeletonCard key={item.uuid} />
                ))}
              </View>
            </View>
          ))}
        ...
        {presentTypes.map(({ type, label }) => (
          <AccordionItem className="border-0" key={type} value={type}>
            ...
            <AccordionContent className="gap-2 p-2">
              {grouped[type].map(renderResourceCard)}
            </AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
